...
namespace serdes
{
    ...
    struct packet
    {
        ...
        template <typename T_pointer, typename eastl::enable_if<eastl::is_pointer<T_pointer>::value, int *>::type = nullptr>
        ...
        /// @param    b_offset: bit offset to start serdes process at
        /// @param    m: starting mode (LOADING/STORING/UNSPECIFIED)
        template <typename T_sized_pointer, typename eastl::enable_if<serdes::detail::is_sized_pointer<T_sized_pointer>::value, int *>::type = nullptr>
        ...
        template <typename T, typename eastl::enable_if<!detail::is_format_modifier<T>::value, int *>::type = nullptr>
        ...
        template <typename T, typename eastl::enable_if<eastl::is_rvalue_reference<T &&>::value && !eastl::is_base_of<packet_base, T>::value, int *>::type = nullptr>
        void load(T &&value, size_t bits = detail::default_bitsize<T>::value)
        {
            (void)value;
            (void)bits;
        ...
        /// @tparam   T: underlying type of the delimited_array
        /// @param    value: referenced delimited_array
        template <typename T, typename eastl::enable_if<eastl::is_base_of<packet_base, T>::value, int *>::type = nullptr>
        ...
        /// @param    bits: number of bits per element
        template <typename T, typename T2, typename eastl::enable_if<!detail::is_format_modifier<T>::value, int *>::type = nullptr>
        ...
        /// @tparam   T2: underlying size type of the array
        /// @param    value: referenced array
        template <typename T, typename T2, typename eastl::enable_if<eastl::is_base_of<packet_base, T>::value, int *>::type = nullptr>
        ...
        template <typename T>
        void add(T &value, const eastl::function<bool()> &validation)
        {
            if (status != status_e::NO_ERROR)
                return;
            if (mode == mode_e::LOADING)
        ...
        /// @param    validation: function that returns true if the field is valid, false to abort the process
        template <typename T>
        void add(T &&value, eastl::function<bool()> &&validation)
        {
...
#pragma GCC diagnostic pop
    ...
    template <typename T_pointer, typename eastl::enable_if<eastl::is_pointer<T_pointer>::value, int *>::type>
    ...
    template <typename T_sized_pointer, typename eastl::enable_if<serdes::detail::is_sized_pointer<T_sized_pointer>::value, int *>::type>
