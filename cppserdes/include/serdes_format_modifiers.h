...
namespace serdes
{
    ...
    struct array
    {
        ...
        template <size_t max_elements>
        array(T (&v)[max_elements], const ST &&s) noexcept : value{&v[0]}, size{eastl::move(s)}, max_size{max_elements} {}
    ...
    namespace detail
    {
        ...
        {
            static constexpr bool value = eastl::is_base_of<packet_base, T>::value;
        };
        template <>
        struct is_format_modifier<formatter> : eastl::true_type
        {
        };
        template <typename ST>
        struct is_format_modifier<pad<ST>> : eastl::true_type
        {
        };
        template <typename ST>
        struct is_format_modifier<align<ST>> : eastl::true_type
        {
        };
        template <typename T, typename ST>
        struct is_format_modifier<bitpack<T, ST>> : eastl::true_type
        {
        };
        template <typename T1, typename T2>
        struct is_format_modifier<array<T1, T2>> : eastl::true_type
        {
        };
        template <typename T>
        struct is_format_modifier<delimited_array<T>> : eastl::true_type
        {
        };
    }
}
